...
def test_rename_columns(run_dtl):
    src = """
    ...
    outputs = run_dtl(src, data=inputs)
    assert outputs["output"] == pa.table({"new_column": [1, 2, 3, 4]})


def test_add_function(run_dtl):
    src = """
    WITH input AS IMPORT 'input';
...
def test_recursive_add_function(run_dtl):
    src = """
    WITH input AS IMPORT 'input';
...
def test_associativity(run_dtl):
    src = """
    WITH input AS IMPORT 'input';
...
def test_join_on_simple(run_dtl):
    src = """
...
def test_join_on_less_simple(run_dtl):
    src = """
    WITH a AS IMPORT 'input_a';
    WITH b AS IMPORT 'input_b';
    WITH output AS
